...
export const Route = createFileRoute("/contact")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Get a free, no-obligation insurance quote in minutes — or speak with an advisor in Boston, MA." },
      { property: "og:title", content: "Contact Golden Trust Insure" },
      { property: "og:description", content: "Get a free quote or talk to an independent insurance advisor." },
...
function ContactPage() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    setSubmitting(false);
    toast.success("Quote request received", { description: `Thanks ${data.name}. An advisor will reach out within one business hour.` });
    reset();
  };
      ...
      <section className="bg-primary text-background pt-36 pb-16 lg:pt-44 lg:pb-20">
        <div className="container-x">
          ...
          <h1 className="font-heading text-4xl sm:text-5xl lg:text-6xl leading-[1.05] max-w-3xl">
            Talk to a real <em className="text-accent not-italic" style={{ fontStyle: "italic" }}>advisor</em>.
          </h1>
          <p className="mt-6 text-background/75 max-w-2xl text-lg">
            No phone trees, no chatbots. Send a message or call us directly — every inquiry is answered by a licensed advisor within one business hour.
          </p>
        </div>
      </section>
          ...
          <div>
            ...
            <ul className="mt-8 space-y-6 text-sm">
              <li className="flex gap-4">
                <span className="flex h-10 w-10 items-center justify-center rounded-full bg-primary text-accent shrink-0"><MapPin className="h-4 w-4" /></span>
                <div>
              ...
              <li className="flex gap-4">
                <span className="flex h-10 w-10 items-center justify-center rounded-full bg-primary text-accent shrink-0"><Phone className="h-4 w-4" /></span>
                <div>
              ...
              <li className="flex gap-4">
                <span className="flex h-10 w-10 items-center justify-center rounded-full bg-primary text-accent shrink-0"><Mail className="h-4 w-4" /></span>
                <div>
              ...
              <li className="flex gap-4">
                <span className="flex h-10 w-10 items-center justify-center rounded-full bg-primary text-accent shrink-0"><Clock className="h-4 w-4" /></span>
                <div>
            ...
            <div className="mt-10 rounded-2xl overflow-hidden aspect-[5/3] bg-[var(--surface-2)] relative">
              ...
              <div className="absolute inset-0 flex items-center justify-center text-center p-6">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Map preview · Click to open in Maps</p>
                </div>
              </div>
            </div>
          </div>

          <form onSubmit={handleSubmit(onSubmit)} className="rounded-2xl bg-card border border-border p-8 lg:p-10">
            ...
            <div className="mt-8 grid gap-5 sm:grid-cols-2">
              ...
              <Field label="Email" error={errors.email?.message}>
                <Input {...register("email")} type="email" placeholder="jane@example.com" className="h-11 rounded-lg" />
              </Field>
              <Field label="Phone" error={errors.phone?.message}>
                <Input {...register("phone")} placeholder="(555) 123-4567" className="h-11 rounded-lg" />
              </Field>
              <Field label="Insurance type" error={errors.type?.message}>
                <Select value={watch("type")} onValueChange={(v) => setValue("type", v, { shouldValidate: true })}>
                  <SelectTrigger className="h-11 rounded-lg w-full"><SelectValue placeholder="Select coverage" /></SelectTrigger>
                  <SelectContent>
                    {services.map((s) => (
            ...
            <div className="mt-5">
              <Field label="Message" error={errors.message?.message}>
                <Textarea {...register("message")} rows={5} placeholder="Tell us about your situation and what you're looking to cover…" className="rounded-lg" />
              </Field>
            </div>

            <Button type="submit" disabled={submitting} size="lg" className="mt-8 rounded-full bg-accent text-accent-foreground hover:bg-accent/90 px-7 h-12 w-full sm:w-auto">
