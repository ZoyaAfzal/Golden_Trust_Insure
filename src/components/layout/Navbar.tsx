    ...
    >
      <div className="container-x flex h-20 items-center justify-between">
        <Link to="/" className="flex items-center gap-2.5 group">
          <span
            className={`flex h-9 w-9 items-center justify-center rounded-full ${transparent ? "bg-accent/90" : "bg-primary"} transition-colors`}
          >
            <Anchor
              className={`h-4.5 w-4.5 ${transparent ? "text-primary" : "text-accent"}`}
              strokeWidth={2.2}
            />
          </span>
          <span
            className={`font-heading text-xl font-semibold tracking-tight ${transparent ? "text-background" : "text-foreground"}`}
          >
            {BRAND.name}
          </span>
        </Link>
        ...
        <div className="flex items-center gap-2">
          <Button
            asChild
            size="lg"
            className="hidden md:inline-flex rounded-full bg-accent text-accent-foreground hover:bg-accent/90 px-5"
          >
          ...
          <Sheet open={open} onOpenChange={setOpen}>
            <SheetTrigger asChild>
              <Button
                variant="ghost"
                size="icon"
                className={`md:hidden ${transparent ? "text-background hover:text-background hover:bg-background/10" : ""}`}
              >
